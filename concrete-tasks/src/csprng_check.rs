...
pub fn command_args() -> clap::Command<'static> {
    let command = clap::Command::new("check_csprng")
        ...
        .arg(
            Arg::new("generated_bytes")
                .short('g')
                .long("generated_bytes")
                .takes_value(true)
                .default_value("4096") // FIXME replace by a realistic but large default value
                .help("Total number of random bytes that has to be generated by concrete-csprng"),
        )
        .arg(
...
pub fn check(args: &ArgMatches) -> Result<(), IoError> {
    let generated_bytes = parse_arg(args, "generated_bytes")?;
    ...
    let result = generate_random_numbers(build_path, nist_tool_path, generated_bytes);
    if result.is_err() {
        return Err(IoError::new(
            ErrorKind::Other,
