# Table of contents

* [What is Concrete-core?](README.md)

## Getting Started

* [General Concepts](getting\_started/general\_concepts.md)
* [Ciphertext Entitites](getting\_started/ciphertext\_entities.md)

## Existing Backends

* [Default backend](backends/backend\_default.md)
* [FFTW backend](backends/backend\_fftw.md)
* [CUDA backend](backends/backend\_cuda.md)

## Adding a new Backend

* [Creating a backend](how\_to/creating\_backends.md)
* [Testing a backend](how\_to/testing\_backends.md)
* [Benchmarking a backend](how\_to/benchmarking\_backends.md)

## Adding a new engine in a backend

* [Creating a new engine](how\_to/creating\_engines.md)
* [Testing a backend](how\_to/testing\_engines.md)
* [Benchmarking a backend](how\_to/benchmarking\_engines.md)

## API reference

* [docs.rs](https://docs.rs/concrete-core/1.0.0-gamma/concrete\_core/index.html)
