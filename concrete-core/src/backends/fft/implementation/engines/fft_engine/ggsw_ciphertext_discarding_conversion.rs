use crate::backends::fft::engines::FftEngine;
use crate::backends::fft::entities::{FftFourierGgswCiphertext32, FftFourierGgswCiphertext64};
use crate::backends::fft::private::crypto::ggsw::{
    fill_with_backward_fourier_scratch, fill_with_forward_fourier_scratch,
};
use crate::backends::fft::private::math::fft::Fft;
use crate::prelude::{FftError, GgswCiphertext32, GgswCiphertext64};
...
impl GgswCiphertextDiscardingConversionEngine<FftFourierGgswCiphertext32, GgswCiphertext32>
    for FftEngine
{
    fn discard_convert_ggsw_ciphertext(
        &mut self,
        output: &mut GgswCiphertext32,
        input: &FftFourierGgswCiphertext32,
    ) -> Result<(), GgswCiphertextDiscardingConversionError<Self::EngineError>> {
        FftError::perform_fft_checks(input.polynomial_size())?;
        GgswCiphertextDiscardingConversionError::perform_generic_checks(output, input)?;
        unsafe { self.discard_convert_ggsw_ciphertext_unchecked(output, input) };
    ...
    unsafe fn discard_convert_ggsw_ciphertext_unchecked(
        ...
        let fft = fft.as_view();
        self.resize(
            fill_with_backward_fourier_scratch(fft)
                .unwrap()
                .unaligned_bytes_required(),
        );
        input
            .0
            .as_view()
            .fill_with_backward_fourier(output.0.as_mut_view(), fft, self.stack());
    }
}

impl GgswCiphertextDiscardingConversionEngine<FftFourierGgswCiphertext64, GgswCiphertext64>
    for FftEngine
{
    /// # Example
    /// ```
    ...
    /// fft_engine.discard_convert_ggsw_ciphertext(&mut fourier_ciphertext, &ciphertext)?;
    ///
    /// fft_engine.discard_convert_ggsw_ciphertext(&mut ciphertext, &fourier_ciphertext)?;
    /// #
    ...
    fn discard_convert_ggsw_ciphertext(
        ...
        input: &FftFourierGgswCiphertext64,
    ) -> Result<(), GgswCiphertextDiscardingConversionError<Self::EngineError>> {
        FftError::perform_fft_checks(input.polynomial_size())?;
        GgswCiphertextDiscardingConversionError::perform_generic_checks(output, input)?;
        unsafe { self.discard_convert_ggsw_ciphertext_unchecked(output, input) };
