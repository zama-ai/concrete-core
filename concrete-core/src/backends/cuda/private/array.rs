...
/// memory, it is pretty close to a `Vec`. That being said, it only present a very very limited
/// API so it is named `Array`.
#[derive(Debug)]
pub struct CudaArray<T: Numeric> {
...
impl<T: Numeric> CudaArray<T> {
    ...
    /// Returns `true` if the CudaArray contains no elements.
    pub fn is_empty(&self) -> bool {
        self.len == 0
    }
}

impl<T: Numeric> Drop for CudaArray<T> {
    fn drop(&mut self) {
