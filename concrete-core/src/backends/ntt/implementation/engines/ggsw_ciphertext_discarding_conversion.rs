use crate::prelude::{
    GgswCiphertext32, GgswCiphertext64, NttEngine, NttFourierGgswCiphertext32,
...
/// # Description:
/// Implementation of [`GgswCiphertextDiscardingConversionEngine`] for [`NttEngine`] that operates
/// on 32 bits integers. It converts a GGSW ciphertext from the standard to the NTT domain.
impl GgswCiphertextDiscardingConversionEngine<GgswCiphertext32, NttFourierGgswCiphertext32>
    for NttEngine
{
    /// # Example
    /// ```
    ...
    ///
    /// let mut fourier_ciphertext: NttFourierGgswCiphertext32 =
    ...
    ///     noise,
    /// )?;
    /// ntt_engine.discard_convert_ggsw_ciphertext(&mut fourier_ciphertext, &ciphertext)?;
    ///
    /// #
    ...
    fn discard_convert_ggsw_ciphertext(
        &mut self,
        output: &mut NttFourierGgswCiphertext32,
...
/// # Description:
/// Implementation of [`GgswCiphertextDiscardingConversionEngine`] for [`NttEngine`] that operates
/// on 64 bits integers. It converts a GGSW ciphertext from the standard to the NTT domain.
impl GgswCiphertextDiscardingConversionEngine<GgswCiphertext64, NttFourierGgswCiphertext64>
    for NttEngine
{
    /// # Example
    /// ```
    ...
    ///     .encrypt_scalar_ggsw_ciphertext(&key_1, &plaintext, noise, level, base_log)?;
    ///
    /// let mut fourier_ciphertext: NttFourierGgswCiphertext64 =
    ///     ntt_engine.convert_ggsw_ciphertext(&ciphertext)?;
    ///
    /// // We're going to re-encrypt and re-convert the input with another secret key
    /// // For this, it is required that the second secret key uses the same GLWE dimension
    ...
    fn discard_convert_ggsw_ciphertext(
        &mut self,
        output: &mut NttFourierGgswCiphertext64,
...
impl<Ciphertext> GgswCiphertextDiscardingConversionEngine<Ciphertext, Ciphertext> for NttEngine
where
