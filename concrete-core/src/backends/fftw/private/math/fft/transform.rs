...
impl Fft {
    ...
    pub fn forward_as_torus<OutCont, InCont, Coef>(
        ...
        buffer: &mut FourierPolynomial<AlignedVec<Complex64>>,
    ) where
        FourierPolynomial<OutCont>: AsMutTensor<Element = Complex64>,
        Polynomial<InCont>: AsRefTensor<Element = Coef>,
        Coef: UnsignedTorus,
    {
        self.forward(fourier_poly, poly, regular_convert_forward_single_torus, buffer);
    }
    ...
    {
        self.forward(fourier_poly, poly, regular_convert_forward_single_integer, buffer);
    }
    ...
    {
        ck_dim_eq!(self.polynomial_size().0 => fourier_poly.polynomial_size().0, poly.polynomial_size().0);
        self.backward(poly, fourier_poly, regular_convert_backward_single_torus, buffer);
    }
        ...
        FourierPolynomial<InCont>: AsMutTensor<Element = Complex64>,
    {
        ...
        for zip_args!(fft_bj, rot_fft_bj) in zip!(b_first.iter(), b_second.iter_mut().rev()) {
        ...
        self.plans.backward(
            ...
            buffer.as_mut_tensor().as_mut_slice(),
        );

        // We fill the polynomial with the conversion function
        convert_function(poly, buffer, &self.correctors.backward)
    }
        ...
        FourierPolynomial<InCont2>: AsMutTensor<Element = Complex64>,
    {
        ...
        for zip_args!(fft_aj, rot_fft_aj, fft_bj) in zip!(
            a_first.iter_mut(),
...
fn replicate_coefficients(fft_a: &mut [Complex64], fft_b: &mut [Complex64], big_n: usize) {
    ...
    for zip_args!(x_i, x_rot_i, y_i) in zip!(
        first_part.iter_mut(),
