...
use crate::prelude::{FftwFourierGlweTensorProductSecretKey32, FftwFourierGlweTensorProductSecretKey64, GlweTensorProductSecretKey32, GlweTensorProductSecretKey64};
use crate::specification::engines::{
    GlweSecretKeyConversionEngine, GlweSecretKeyConversionError,
};
...
/// domain.
impl GlweSecretKeyConversionEngine<FftwFourierGlweTensorProductSecretKey32, 
    GlweTensorProductSecretKey32> for 
FftwEngine {
    /// # Example
    /// ```
    ...
    /// let fourier_tensor_product_key = fftw_engine.create_tensor_product_glwe_secret_key_same_key
    /// (&key)?;
    ///
    ...
    /// #
    /// assert_eq!(tensor_product_key.glwe_dimension(), glwe_dimension);
    ...
    unsafe fn convert_glwe_secret_key_unchecked(
        ...
        let mut output =
             GlweSecretKey::tensor_product_from_container(vec![0_u32; input.glwe_dimension
             ().0 
                 * input.polynomial_size().0], input.polynomial_size());
...
/// domain.
impl GlweSecretKeyConversionEngine<FftwFourierGlweTensorProductSecretKey64,
    GlweTensorProductSecretKey64> for
FftwEngine {
    /// # Example
    /// ```
    /// use concrete_commons::dispersion::Variance;
    ...
    unsafe fn convert_glwe_secret_key_unchecked(
        ...
        let mut output =
            GlweSecretKey::tensor_product_from_container(vec![0_u64; input.glwe_dimension().0
                * input.polynomial_size().0], input.polynomial_size());
