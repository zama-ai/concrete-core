...
impl From<FftwError> for LweCiphertextDiscardingBootstrapError<FftwError> {
...
/// # Description:
/// Implementation of [`LweCiphertextDiscardingBootstrapEngine`] for [`FftwEngine`] that operates on
/// 32 bits integers.
impl
    ...
    > for FftwEngine
{
    ...
    ///
    /// fftw_engine.discard_bootstrap_lwe_ciphertext(&mut output, &input, &acc, &bsk)?;
    /// #
    ...
    > for FftwEngine
{
    ...
    /// # fn main() -> Result<(), Box<dyn Error>> {
    /// // Here a hard-set encoding is applied (shift by 20 bits)
    /// use concrete_core::backends::fftw::engines::FftwEngine;
    /// use concrete_core::backends::fftw::entities::FftwFourierLweBootstrapKey32;
    /// let input = 3_u32 << 20;
    /// // DISCLAIMER: the parameters used here are only for test purpose, and are not secure.
    /// let (lwe_dim, lwe_dim_output, glwe_dim, poly_size) = (
    ///     LweDimension(4),
