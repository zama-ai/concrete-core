...
use crate::prelude::{FftwFourierGlweTensorProductCiphertext32, FftwFourierGlweTensorProductCiphertext64, GlweCiphertext32, GlweCiphertext64, GlweTensorProductCiphertext32, GlweTensorProductCiphertext64};
...
/// domain.
impl GlweCiphertextConversionEngine<FftwFourierGlweTensorProductCiphertext32, 
    GlweTensorProductCiphertext32> for FftwEngine {
    /// # Example
    /// ```
    /// use concrete_commons::dispersion::Variance;
    /// use concrete_commons::parameters::{
    ...
    /// let fourier_ciphertext: FftwFourierGlweCiphertext32 =
    ///     fftw_engine.convert_glwe_ciphertext(&ciphertext)?;
    /// // Compute the tensor product between the ciphertext and itself
    /// // The result is in the Fourier domain
    /// let tensor_product_ciphertext = fftw_engine.tensor_product_glwe_ciphertext_same_key
    /// (&fourier_ciphertext, &fourier_ciphertext)?;
    /// // Then we convert the result back to the standard domain.
    /// let ciphertext_out: GlweCiphertext32 =
    ///     fftw_engine.convert_glwe_ciphertext(&tensor_product_ciphertext)?;
    ///
    /// assert_eq!(ciphertext_out.glwe_dimension(), glwe_dimension);
    /// assert_eq!(ciphertext_out.polynomial_size(), polynomial_size);
    ///
    ...
    fn convert_glwe_ciphertext(
        &mut self,
        input: &FftwFourierGlweTensorProductCiphertext32,
    ) -> Result<GlweTensorProductCiphertext32, GlweCiphertextConversionError<Self::EngineError>> {
        if !ALLOWED_POLY_SIZE.contains(&input.polynomial_size().0) {
            return Err(GlweCiphertextConversionError::from(
                FftwError::UnsupportedPolynomialSize,
            ));
...
/// # Description:
/// Implementation of [`GlweCiphertextConversionEngine`] for [`FftwEngine`] that operates on
/// 64 bits integers. It converts a tensor product GLWE ciphertext from the Fourier to the standard 
/// domain.
impl GlweCiphertextConversionEngine<FftwFourierGlweTensorProductCiphertext64,
    GlweTensorProductCiphertext64> for FftwEngine {
    /// # Example
    /// ```
    /// use concrete_commons::dispersion::Variance;
    /// use concrete_commons::parameters::{
    ...
    fn convert_glwe_ciphertext(
        &mut self,
        input: &FftwFourierGlweTensorProductCiphertext64,
    ) -> Result<GlweTensorProductCiphertext64, GlweCiphertextConversionError<Self::EngineError>> {
        if !ALLOWED_POLY_SIZE.contains(&input.polynomial_size().0) {
            return Err(GlweCiphertextConversionError::from(
                FftwError::UnsupportedPolynomialSize,
            ));
