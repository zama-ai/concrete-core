...
use crate::prelude::{FourierBufferKey, GlweSize, PolynomialSize};

/// The error which can occur in the execution of FHE operations, due to the fftw implementation.
#[derive(Debug)]
pub enum FftwParallelError {
    UnsupportedPolynomialSize,
}

impl Display for FftwParallelError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            FftwParallelError::UnsupportedPolynomialSize => {
                write!(
                    f,
...
impl Error for FftwParallelError {}

/// The main engine exposed by the fftw backend.
pub struct FftwParallelEngine {
...
impl AbstractEngineSeal for FftwParallelEngine {}

impl AbstractEngine for FftwParallelEngine {
    type EngineError = FftwParallelError;

    type Parameters = ();

    fn new(_parameters: Self::Parameters) -> Result<Self, Self::EngineError> {
        Ok(FftwParallelEngine {
