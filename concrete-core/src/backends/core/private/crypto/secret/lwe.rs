    ...
    Kind: KeyKind,
{
    ...
    /// secret_key.encrypt_seeded_lwe(&mut encrypted, &plain, noise);
    /// ```
    ...
    /// secret_key.encrypt_seeded_lwe_list(&mut encrypted_values, &plain_values, noise);
    /// ```
    ...
    /// );
    /// let noise = LogStandardDev::from_log_standard_dev(-15.);
    /// secret_key.encrypt_constant_seeded_gsw(&mut ciphertext, &Plaintext(10), noise);
    /// ```
    ...
    /// );
    /// let noise = LogStandardDev::from_log_standard_dev(-15.);
    /// secret_key.par_encrypt_constant_seeded_gsw(&mut ciphertext, &Plaintext(10), noise);
    /// ```
    #[cfg(feature = "multithread")]
