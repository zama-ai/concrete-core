    ...
    Kind: KeyKind,
{
    ...
    /// secret_key.encrypt_seeded_glwe(&mut ciphertext, &plaintexts, noise);
    /// ```
    ...
    ///     CiphertextCount(2),
    /// );
    /// secret_key.encrypt_seeded_glwe_list(&mut ciphertexts, &plaintexts, noise);
    /// ```
    ...
    /// );
    /// let noise = LogStandardDev::from_log_standard_dev(-15.);
    /// secret_key.encrypt_constant_seeded_ggsw(&mut ciphertext, &Plaintext(10), noise);
    /// ```
    ...
    /// let noise = LogStandardDev::from_log_standard_dev(-15.);
    /// let mut encryption_generator = EncryptionRandomGenerator::new(None);
    /// secret_key.par_encrypt_constant_seeded_ggsw(&mut ciphertext, &Plaintext(10), noise);
    /// ```
    #[cfg(feature = "multithread")]
