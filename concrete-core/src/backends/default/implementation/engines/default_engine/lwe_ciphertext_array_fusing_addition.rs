...
use crate::specification::engines::{
    LweCiphertextArrayFusingAdditionEngine, LweCiphertextArrayFusingAdditionError,
};

/// # Description:
/// Implementation of [`LweCiphertextArrayFusingAdditionEngine`] for [`DefaultEngine`]
/// that operates on 32 bits integers.
impl LweCiphertextArrayFusingAdditionEngine<LweCiphertextArray32, LweCiphertextArray32>
    for DefaultEngine
{
    /// # Example:
    /// ```
    ...
    ///
    /// engine.fuse_add_lwe_ciphertext_array(&mut output_ciphertext_array, &ciphertext_array)?;
    /// #
    ...
    fn fuse_add_lwe_ciphertext_array(
        ...
        LweCiphertextArrayFusingAdditionError::perform_generic_checks(output, input)?;
        unsafe { self.fuse_add_lwe_ciphertext_array_unchecked(output, input) };
        Ok(())
    }
...
impl LweCiphertextArrayFusingAdditionEngine<LweCiphertextArray64, LweCiphertextArray64>
    for DefaultEngine
{
    /// # Example:
    /// ```
