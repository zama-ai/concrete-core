...
use crate::specification::engines::{
    LweCiphertextArrayEncryptionEngine, LweCiphertextArrayEncryptionError,
};
use crate::specification::entities::{LweSecretKeyEntity, PlaintextArrayEntity};

/// # Description:
/// Implementation of [`LweCiphertextArrayEncryptionEngine`] for [`DefaultEngine`] that operates on
/// 32 bits integers.
impl LweCiphertextArrayEncryptionEngine<LweSecretKey32, PlaintextArray32, LweCiphertextArray32>
    for DefaultEngine
{
    /// # Example:
    /// ```
    ...
    fn encrypt_lwe_ciphertext_array(
        ...
        noise: Variance,
    ) -> Result<LweCiphertextArray32, LweCiphertextArrayEncryptionError<Self::EngineError>> {
        Ok(unsafe { self.encrypt_lwe_ciphertext_array_unchecked(key, input, noise) })
    }
    ...
    unsafe fn encrypt_lwe_ciphertext_array_unchecked(
        ...
        key.0
            .encrypt_lwe_list(&mut array, &input.0, noise, &mut self.encryption_generator);
...
/// Implementation of [`LweCiphertextArrayEncryptionEngine`] for [`DefaultEngine`] that operates on
/// 64 bits integers.
impl LweCiphertextArrayEncryptionEngine<LweSecretKey64, PlaintextArray64, LweCiphertextArray64>
    for DefaultEngine
{
    /// # Example:
    /// ```
    ...
    fn encrypt_lwe_ciphertext_array(
        ...
        noise: Variance,
    ) -> Result<LweCiphertextArray64, LweCiphertextArrayEncryptionError<Self::EngineError>> {
        Ok(unsafe { self.encrypt_lwe_ciphertext_array_unchecked(key, input, noise) })
    }
