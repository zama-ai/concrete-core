...
use crate::specification::entities::{
    LweBootstrapKeyEntity, LweCiphertextEntity,
...
engine_error! {
    LweCiphertextDiscardingWopPbsError for
...
impl<EngineError: std::error::Error>
    LweCiphertextDiscardingWopPbsError<EngineError>
{
    ...
    >(
        ...
        if luts.plaintext_count().0 % lut_polynomial_size != 0 {
            return Err(Self::MalformedLookUpTables);
        }

        let small_lut_size = luts.plaintext_count().0;
...
///
/// # Formal Definition
pub trait LweCiphertextDiscardingWopPbsEngine<
    Input,
    Output,
    BootstrapKey,
    KeyswitchKey,
        ...
        LweCircuitBootstrapPrivateFunctionalPackingKeyswitchKeysEntity,
{
    ...
    /// of [`LweCiphertextDiscardingWopPbsError`]. For safety
