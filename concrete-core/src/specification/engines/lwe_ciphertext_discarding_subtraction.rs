...
/// the subtraction of the `input_2` LWE ciphertext from the `input_1` LWE ciphertext.
///
/// # Formal Definition
...
where
    ...
    OutputCiphertext: LweCiphertextEntity<KeyDistribution = InputCiphertext::KeyDistribution>,
{
    /// Subtracts the `input2` ciphertext from the `input1` ciphertext.
    ...
    /// Unsafely subtracts two LWE ciphertexts.
    ///
    /// # Safety
