...
//! the same length:
//! ```
//! use concrete_core::commons::math::tensor::Tensor;
//! // We allocate two tensors of size 10
//! let mut tensor1 = Tensor::allocate(5. as u32, 10);
//! let tensor2 = Tensor::allocate(3. as u32, 10);
...
macro_rules! tensor_traits {
    ($Type:ident) => {
        impl<Element, Cont> $crate::commons::math::tensor::AsRefTensor for $Type<Cont>
        where
            Cont: $crate::commons::math::tensor::AsRefSlice<Element = Element>,
        {
        ...
        impl<Element, Cont> $crate::commons::math::tensor::AsMutTensor for $Type<Cont>
        where
            Cont: $crate::commons::math::tensor::AsMutSlice<Element = Element>,
        {
            type Element = Element;
            type Container = Cont;
            fn as_mut_tensor(
                &mut self,
            ) -> &mut Tensor<<Self as $crate::commons::math::tensor::AsMutTensor>::Container> {
                &mut self.tensor
            }
        }

        impl<Cont> $crate::commons::math::tensor::IntoTensor for $Type<Cont>
        where
            Cont: $crate::commons::math::tensor::AsRefSlice,
        {
            type Element = <Cont as $crate::commons::math::tensor::AsRefSlice>::Element;
            type Container = Cont;
...
macro_rules! ck_dim_eq {
    ($tensor_size: expr => $($size: expr),* ) => {
        let func = $crate::commons::math::tensor::current_func_path!();
        $(
