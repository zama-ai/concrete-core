...
use concrete_commons::parameters::{GlweDimension, MonomialIndex, PolynomialSize};
use concrete_core::prelude::{
    GlweCiphertextEntity, LweCiphertextSampleExtractionEngine, LweCiphertextEntity,
};

/// A fixture for the types implementing the `LweCiphertextSampleExtractionEngine` trait.
pub struct LweCiphertextSampleExtractionFixture;

#[derive(Debug)]
pub struct LweCiphertextSampleExtractionParameters {
    pub noise: Variance,
...
for LweCiphertextSampleExtractionFixture
    where
        Precision: IntegerPrecision,
        Engine: LweCiphertextSampleExtractionEngine<GlweCiphertext, LweCiphertext>,
...
{
    type Parameters = LweCiphertextSampleExtractionParameters;
    ...
    fn generate_parameters_iterator() -> Box<dyn Iterator<Item = Self::Parameters>> {
        Box::new(
            vec![LweCiphertextSampleExtractionParameters {
    ...
    fn execute_engine(
        ...
        let lwe_ciphertext = unsafe {
            engine.sample_extract_lwe_ciphertext_unchecked(
                &glwe_ciphertext,
                parameters.nth,
            )
        };
    ...
    fn compute_criteria(
        ...
        _maker: &mut Maker,
        _repetition_proto: &Self::RepetitionPrototypes,
    ) -> Self::Criteria { (parameters.noise,) }
