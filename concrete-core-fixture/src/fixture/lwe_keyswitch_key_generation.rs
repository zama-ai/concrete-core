...
use concrete_core::prelude::{
    LweKeyswitchKeyEntity, LweKeyswitchKeyGenerationEngine, LweSecretKeyEntity,
};

/// A fixture for the types implementing the `LweKeyswitchKeyGenerationEngine` trait.
pub struct LweKeyswitchKeyGenerationFixture;

#[derive(Debug)]
pub struct LweKeyswitchKeyGenerationParameters {
    pub noise: Variance,
...
impl<
    ...
    Fixture<
        ...
        Engine,
        (InputSecretKey, OutputSecretKey, LweKeyswitchKey),
    > for LweKeyswitchKeyGenerationFixture
where
    Precision: IntegerPrecision,
    InputKeyDistribution: KeyDistributionMarker,
    OutputKeyDistribution: KeyDistributionMarker,
    Engine: LweKeyswitchKeyGenerationEngine<InputSecretKey, OutputSecretKey, LweKeyswitchKey>,
...
{
    type Parameters = LweKeyswitchKeyGenerationParameters;
    ...
    fn generate_parameters_iterator() -> Box<dyn Iterator<Item = Self::Parameters>> {
        Box::new(
            vec![LweKeyswitchKeyGenerationParameters {
    ...
    fn execute_engine(
        ...
        let ksk = unsafe {
            engine.generate_new_lwe_keyswitch_key_unchecked(
                &sk_in,
                &sk_out,
                parameters.level,
                parameters.base_log,
