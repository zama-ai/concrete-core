...
use crate::generation::prototyping::{
    PrototypesCleartextArray, PrototypesGlweCiphertext, PrototypesGlweSecretKey,
    PrototypesLweCiphertextArray, PrototypesLwePrivateFunctionalPackingKeyswitchKey,
    PrototypesLweSecretKey, PrototypesPlaintextArray,
};
use crate::generation::synthesizing::{
    SynthesizesCleartextArray, SynthesizesGlweCiphertext, SynthesizesLweCiphertextArray,
...
use concrete_core::prelude::{
    BinaryKeyKind, CleartextArrayEntity, DecompositionBaseLog, DecompositionLevelCount,
    DispersionParameter, GaussianKeyKind, GlweCiphertextEntity, GlweDimension, LogStandardDev,
    LweCiphertextArrayEntity,
    LweCiphertextArrayGlweCiphertextDiscardingPrivateFunctionalPackingKeyswitchEngine,
    LweCiphertextCount, LweDimension, LwePrivateFunctionalPackingKeyswitchKeyEntity,
    PolynomialSize, StandardDev, TernaryKeyKind, Variance,
};
use std::any::TypeId;

/// A fixture for the types implementing the
/// `LweCiphertextArrayGlweCiphertextDiscardingPrivateFunctionalPackingKeyswitchEngine` trait.
pub struct LweCiphertextArrayGlweCiphertextDiscardingPrivateFunctionalPackingKeyswitchFixture;

#[derive(Debug)]
pub struct LweCiphertextArrayGlweCiphertextDiscardingPrivateFunctionalPackingKeyswitchParameters {
...
impl<
    ...
    > for LweCiphertextArrayGlweCiphertextDiscardingPrivateFunctionalPackingKeyswitchFixture
where
    Precision: IntegerPrecision,
    InputKeyDistribution: KeyDistributionMarker,
    OutputKeyDistribution: KeyDistributionMarker,
    Engine: LweCiphertextArrayGlweCiphertextDiscardingPrivateFunctionalPackingKeyswitchEngine<
        LwePrivateFunctionalPackingKeyswitchKey,
        InputCiphertextArray,
        OutputCiphertext,
    >,
    ...
    Maker: SynthesizesLweCiphertextArray<Precision, InputKeyDistribution, InputCiphertextArray>
...
{
    type Parameters =
        LweCiphertextArrayGlweCiphertextDiscardingPrivateFunctionalPackingKeyswitchParameters;
    ...
    fn generate_random_sample_prototypes(
        ...
        let raw_plaintext_array = Precision::Raw::uniform_vec(parameters.input_lwe_count.0);
    ...
    fn prepare_context(
        ...
        let (_, proto_input_ciphertext_array, proto_output_ciphertext) = sample_proto;
    ...
    fn execute_engine(
        ...
        let (mut output_ciphertext, input_ciphertext_array, pfpksk) = context;
        unsafe {
            engine.discard_private_functional_packing_keyswitch_lwe_ciphertext_array_unchecked(
                &mut output_ciphertext,
                &input_ciphertext_array,
                &pfpksk,
            );
        };
        (output_ciphertext, input_ciphertext_array, pfpksk)
    }
    ...
    fn process_context(
        ...
        let (_, _, proto_output_secret_key, proto_cleartext_array) = repetition_proto;
        let (proto_plaintext_array, ..) = sample_proto;
        let proto_output_ciphertext = maker.unsynthesize_glwe_ciphertext(output_ciphertext);
        let proto_output_plaintext = maker.decrypt_glwe_ciphertext_to_plaintext_array(
            proto_output_secret_key,
            &proto_output_ciphertext,
        );
        let raw_cleartext_array = maker.transform_cleartext_array_to_raw_vec(proto_cleartext_array);
        let function_poly = Polynomial::from_container(raw_cleartext_array);
        let raw_input_array = maker.transform_plaintext_array_to_raw_vec(proto_plaintext_array);
        let input_poly = Polynomial::from_container(raw_input_array);
        let mut raw_result =
        ...
        maker.destroy_lwe_ciphertext_array(input_ciphertext);
        ...
        (
            ...
            maker
                .transform_plaintext_array_to_raw_vec(&proto_output_plaintext)
                .iter()
                .take(parameters.output_polynomial_size.0)
                .cloned()
                .collect::<Vec<Precision::Raw>>(),
