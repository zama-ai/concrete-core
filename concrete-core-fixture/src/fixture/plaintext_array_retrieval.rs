...
use concrete_core::prelude::{PlaintextArrayEntity, PlaintextArrayRetrievalEngine};

/// A fixture for the types implementing the `PlaintextArrayRetrievalEngine` trait.
pub struct PlaintextArrayRetrievalFixture;

#[derive(Debug)]
pub struct PlaintextArrayRetrievalParameters {
    count: PlaintextCount,
}

impl<Precision, Engine, PlaintextArray> Fixture<Precision, (), Engine, (PlaintextArray,)>
    for PlaintextArrayRetrievalFixture
where
    Precision: IntegerPrecision,
    Engine: PlaintextArrayRetrievalEngine<PlaintextArray, Precision::Raw>,
    PlaintextArray: PlaintextArrayEntity,
    Maker: SynthesizesPlaintextArray<Precision, PlaintextArray>,
{
    type Parameters = PlaintextArrayRetrievalParameters;
    ...
    fn generate_parameters_iterator() -> Box<dyn Iterator<Item = Self::Parameters>> {
        Box::new(
            vec![
                PlaintextArrayRetrievalParameters {
    ...
    fn execute_engine(
        ...
        let raw_output_array =
            unsafe { engine.retrieve_plaintext_array_unchecked(&plaintext_array) };
        (plaintext_array, raw_output_array)
    }
