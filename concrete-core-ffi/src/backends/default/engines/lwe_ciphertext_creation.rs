...
/// Create an `LweCiphertextView64` from a raw pointer.
///
...
pub unsafe extern "C" fn default_engine_create_lwe_ciphertext_view_u64(
    ...
    input: *const u64,
    lwe_size: usize,
    result: *mut *mut LweCiphertextView64,
) -> c_int {
    catch_panic(|| {
...
/// Create an `LweCiphertextMutView64` from a raw pointer.
///
/// Refer to `concrete-core` implementation for detailed documentation.
///
/// This function is [checked](crate#safety-checked-and-unchecked-functions).
...
pub unsafe extern "C" fn default_engine_create_lwe_ciphertext_mut_view_u64(
    ...
    input: *mut u64,
    lwe_size: usize,
    result: *mut *mut LweCiphertextMutView64,
) -> c_int {
    catch_panic(|| {
        check_ptr_is_non_null_and_aligned(result).unwrap();
