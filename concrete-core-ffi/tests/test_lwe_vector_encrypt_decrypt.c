...
void encrypt_lwe_vector_decrypt_view_buffers_test(void) {
  ...
  uint64_t *input_ct_vector_buffer = aligned_alloc(U64_ALIGNMENT, sizeof(uint64_t) * (lwe_dimension + 1)
  * lwe_count);
  uint64_t *output_ct_vector_buffer = aligned_alloc(U64_ALIGNMENT, sizeof(uint64_t) * (lwe_dimension + 
  1) * lwe_count);

  LweCiphertextVectorView64 *input_ct_vector_as_view = NULL;
  int input_ct_vector_as_view_ok = default_engine_create_lwe_ciphertext_vector_view_from_u64(
      engine, input_ct_vector_buffer, lwe_dimension + 1, lwe_count, &input_ct_vector_as_view);
  assert(input_ct_vector_as_view_ok == 0);

  LweCiphertextVectorMutView64 *input_ct_vector_as_mut_view = NULL;
  int input_ct_vector_as_mut_view_ok = default_engine_create_lwe_ciphertext_vector_mut_view_from_u64(
  ...
  LweCiphertextVectorView64 *output_ct_vector_as_view = NULL;
  int output_ct_vector_as_view_ok = default_engine_create_lwe_ciphertext_vector_view_from_u64(
      engine, output_ct_vector_buffer, lwe_dimension + 1, lwe_count, &output_ct_vector_as_view);
  assert(output_ct_vector_as_view_ok == 0);

  uint64_t *plaintext_vector =
      aligned_alloc(U64_ALIGNMENT, sizeof(uint64_t) * lwe_count);
  for (uint i = 0; i < lwe_count; i++) {
    plaintext_vector[i] = ((uint64_t) 1) << SHIFT;
  }
  double variance = 0.000;

  // We encrypt the plaintext
  int enc_input_ct_vector_ok = default_engine_discard_encrypt_lwe_ciphertext_vector_u64_view_buffers(
  ...
  uint64_t *output = aligned_alloc(U64_ALIGNMENT, sizeof(uint64_t) * lwe_count);
  ...
  destroy_lwe_ciphertext_vector_view_u64(input_ct_vector_as_view);
  destroy_lwe_ciphertext_vector_mut_view_u64(input_ct_vector_as_mut_view);
  destroy_lwe_ciphertext_vector_view_u64(output_ct_vector_as_view);
...
void encrypt_lwe_vector_decrypt_unchecked_view_buffers_test(void) {
  ...
  LweCiphertextVectorView64 *input_ct_vector_as_view = NULL;
  int input_ct_vector_as_view_ok = default_engine_create_lwe_ciphertext_vector_view_from_unchecked_u64(
      engine, input_ct_vector_buffer, lwe_dimension + 1, lwe_count, &input_ct_vector_as_view);
  assert(input_ct_vector_as_view_ok == 0);

  LweCiphertextVectorMutView64 *input_ct_vector_as_mut_view = NULL;
  int input_ct_vector_as_mut_view_ok = default_engine_create_lwe_ciphertext_vector_mut_view_from_unchecked_u64(
      engine, input_ct_vector_buffer, lwe_dimension + 1, lwe_count, &input_ct_vector_as_mut_view);
  assert(input_ct_vector_as_mut_view_ok == 0);

  LweCiphertextVectorView64 *output_ct_vector_as_view = NULL;
  int output_ct_vector_as_view_ok = default_engine_create_lwe_ciphertext_vector_view_from_unchecked_u64(
      engine, output_ct_vector_buffer, lwe_dimension + 1, lwe_count, &output_ct_vector_as_view);
  assert(output_ct_vector_as_view_ok == 0);
  ...
  int enc_input_ct_vector_ok = 
      default_engine_discard_encrypt_lwe_ciphertext_vector_unchecked_u64_view_buffers(
      engine, sk, input_ct_vector_as_mut_view, plaintext_vector, variance, lwe_count);
  assert(enc_input_ct_vector_ok == 0);
  ...
  destroy_lwe_ciphertext_vector_view_unchecked_u64(input_ct_vector_as_view);
  destroy_lwe_ciphertext_vector_mut_view_unchecked_u64(input_ct_vector_as_mut_view);
  destroy_lwe_ciphertext_vector_view_unchecked_u64(output_ct_vector_as_view);
...
void encrypt_lwe_vector_decrypt_raw_ptr_buffers_test(void) {
  ...
  uint64_t *input_ct_vector_buffer = aligned_alloc(U64_ALIGNMENT, sizeof(uint64_t)
  ...
  // We encrypt the plaintext
  int enc_input_ct_vector_ok = default_engine_discard_encrypt_lwe_ciphertext_vector_u64_raw_ptr_buffers(
...
void encrypt_lwe_vector_decrypt_unchecked_raw_ptr_buffers_test(void) {
  ...
  // We encrypt the plaintext
  int enc_input_ct_vector_ok = default_engine_discard_encrypt_lwe_ciphertext_vector_unchecked_u64_raw_ptr_buffers(
      engine, sk, input_ct_vector_buffer, plaintext_vector, variance, lwe_count);
  assert(enc_input_ct_vector_ok == 0);
