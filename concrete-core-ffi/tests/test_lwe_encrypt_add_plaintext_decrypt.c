...
void add_plaintext_view_buffers_test(void) {
  ...
  uint32_t *input_ct_buffer = aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * (lwe_dimension + 1));
  ...
  int input_ct_as_view_ok = default_engine_create_lwe_ciphertext_view_from_u32(
  ...
  uint32_t added_plaintext = ((uint32_t)10) << SHIFT;
  double variance = 0.000;

  // We encrypt the plaintext
  int enc_input_ct_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_view_buffers(
      engine, sk, input_ct_as_mut_view, plaintext, variance);
  assert(enc_input_ct_ok == 0);

  int add_ok = default_engine_discard_add_lwe_ciphertext_plaintext_u32_view_buffers(
...
void add_plaintext_unchecked_view_buffers_test(void) {
  ...
  LweCiphertextView32 *input_ct_as_view = NULL;
  int input_ct_as_view_ok = default_engine_create_lwe_ciphertext_view_from_unchecked_u32(
      engine, input_ct_buffer, lwe_dimension + 1, &input_ct_as_view);
  assert(input_ct_as_view_ok == 0);
  ...
  // We encrypt the plaintext
  int enc_input_ct_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_view_buffers(
      engine, sk, input_ct_as_mut_view, plaintext, variance);
  assert(enc_input_ct_ok == 0);

  int add_ok = default_engine_discard_add_lwe_ciphertext_plaintext_unchecked_u32_view_buffers(
      engine, output_ct_as_mut_view, input_ct_as_view, added_plaintext);
  assert(add_ok == 0);
...
void add_plaintext_raw_ptr_buffers_test(void) {
  ...
  // We encrypt the plaintext
  int enc_input_ct_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_raw_ptr_buffers(
      engine, sk, input_ct_buffer, plaintext, variance);
  assert(enc_input_ct_ok == 0);

  int add_ok = default_engine_discard_add_lwe_ciphertext_plaintext_u32_raw_ptr_buffers(
...
void add_plaintext_unchecked_raw_ptr_buffers_test(void) {
  ...
  // We encrypt the plaintext
  int enc_input_ct_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_raw_ptr_buffers(
      engine, sk, input_ct_buffer, plaintext, variance);
  assert(enc_input_ct_ok == 0);

  int add_ok = default_engine_discard_add_lwe_ciphertext_plaintext_unchecked_u32_raw_ptr_buffers(
      engine, output_ct_buffer, input_ct_buffer, lwe_dimension, added_plaintext);
  assert(add_ok == 0);
