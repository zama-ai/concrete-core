...
void convert_bootstrap_view_buffers_test(void) {
...
void convert_keyswitch_view_buffers_test(void) {
  // We generate the random sources
  DefaultEngine *engine = NULL;
  CudaEngine *cuda_engine = NULL;
  SeederBuilder *builder = get_best_seeder();
  ...
  int ksk_ok = default_engine_create_lwe_keyswitch_key_u64(
      engine, input_sk, output_sk, level, base_log, ksk_variance, ksk);
  assert(ksk_ok == 0);

  // We convert backward and forward
  CudaLweKeyswitchKey64 *d_ksk = NULL;
  int convert_ok = cuda_engine_convert_lwe_keyswitch_key_to_cuda_lwe_keyswitch_key_u64(
          cuda_engine, ksk, d_ksk);
  assert(convert_ok == 0);

  LweKeyswitchKey64 *output_ksk = NULL;
  convert_ok = default_engine_create_lwe_keyswitch_key_u64(
      engine, input_sk, output_sk, level, base_log, ksk_variance, output_ksk);
  assert(convert_ok == 0);

  CudaLweKeyswitchKey64 *d_ksk = NULL;
  int convert_ok = cuda_engine_convert_cuda_lwe_keyswitch_key_to_lwe_keyswitch_key_u64(
          cuda_engine, d_ksk, output_ksk);
  assert(convert_ok == 0);
  ...
  default_engine_destroy_lwe_keyswitch_key_u64(engine, output_ksk);
  cuda_engine_destroy_cuda_lwe_keyswitch_u64(engine, d_ksk);
  destroy_default_engine(engine);
  destroy_cuda_engine(cuda_engine);
  destroy_seeder_builder(builder);
}
int main(void) {
    convert_bootstrap_view_buffers_test();
    convert_keyswitch_view_buffers_test();
  return EXIT_SUCCESS;
}
