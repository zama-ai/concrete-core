...
void bootstrap_key_to_mut_view_conversion_view_buffers(void) {
  ...
  LweBootstrapKey32 *standard_bsk = NULL;
  int seeded_bsk_ok = default_parallel_engine_generate_new_lwe_bootstrap_key_u32(
  ...
  uint32_t *bootstrap_key_mut_mem = aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * bsk_buffer_len);

  for (size_t idx = 0; idx < bsk_buffer_len; ++idx) {
    bootstrap_key_mut_mem[idx] = 0;
  }

  LweBootstrapKeyMutView32 *standard_bsk_mut_view = NULL;
  int bsk_mut_view_ok = default_engine_create_lwe_bootstrap_key_mut_view_from_u32(
  ...
  int bsk_to_bsk_mut_view_conversion =
      default_engine_discard_convert_lwe_bootstrap_key_to_lwe_bootstrap_key_mut_view_u32_view_buffers(
  ...
  destroy_lwe_bootstrap_key_u32(standard_bsk);
  destroy_lwe_bootstrap_key_mut_view_u32(standard_bsk_mut_view);
...
void bootstrap_key_to_mut_view_conversion_unchecked_view_buffers(void) {
  ...
  LweBootstrapKey32 *standard_bsk = NULL;
  int seeded_bsk_ok = default_parallel_engine_generate_new_lwe_bootstrap_key_unchecked_u32(
      default_parallel_engine, input_lwe_sk, output_glwe_sk, base_log, level, pbs_variance,
      &standard_bsk);
  assert(seeded_bsk_ok == 0);
  ...
  LweBootstrapKeyMutView32 *standard_bsk_mut_view = NULL;
  int bsk_mut_view_ok = default_engine_create_lwe_bootstrap_key_mut_view_from_unchecked_u32(
      default_engine, bootstrap_key_mut_mem, input_lwe_dimension, glwe_size, poly_size, base_log,
      level, &standard_bsk_mut_view);
  assert(bsk_mut_view_ok == 0);

  int bsk_to_bsk_mut_view_conversion =
      default_engine_discard_convert_lwe_bootstrap_key_to_lwe_bootstrap_key_mut_view_unchecked_u32_view_buffers(
          default_engine, standard_bsk, standard_bsk_mut_view);
  assert(bsk_to_bsk_mut_view_conversion == 0);
  ...
  destroy_lwe_bootstrap_key_unchecked_u32(standard_bsk);
  destroy_lwe_bootstrap_key_mut_view_unchecked_u32(standard_bsk_mut_view);
...
void bootstrap_key_to_mut_view_conversion_raw_ptr_buffers(void) {
  ...
  int bsk_to_bsk_mut_view_conversion =
      default_engine_discard_convert_lwe_bootstrap_key_to_lwe_bootstrap_key_mut_view_u32_raw_ptr_buffers(
...
void bootstrap_key_to_mut_view_conversion_unchecked_raw_ptr_buffers(void) {
  ...
  int bsk_to_bsk_mut_view_conversion =
      default_engine_discard_convert_lwe_bootstrap_key_to_lwe_bootstrap_key_mut_view_unchecked_u32_raw_ptr_buffers(
          default_engine, standard_bsk, bootstrap_key_mut_mem);
  assert(bsk_to_bsk_mut_view_conversion == 0);
