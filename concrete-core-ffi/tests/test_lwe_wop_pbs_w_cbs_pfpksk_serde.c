...
void lwe_cbs_vp_view_buffers_test(void) {
  ...
  int lwe_small_sk_ok =
      default_engine_generate_new_lwe_secret_key_u32(default_engine, lwe_dimension, &lwe_small_sk);
  ...
  LweCircuitBootstrapPrivateFunctionalPackingKeyswitchKeys32 *cbs_pfpksk = NULL;
  int cbs_pfpksk_ok =
  ...
  LweCircuitBootstrapPrivateFunctionalPackingKeyswitchKeys32 *cbs_pfpksk_deser = NULL;
  ...
  uint32_t mask = ((uint32_t)1 << number_of_bits_per_ct) - 1;
  uint32_t lsb = val & mask;
  uint32_t msb = (val >> number_of_bits_per_ct) & mask;

  size_t delta_log_ciphertext = 32 - number_of_bits_per_ct;

  uint32_t msb_encoded = msb << delta_log_ciphertext;
  uint32_t lsb_encoded = lsb << delta_log_ciphertext;

  printf("msb: %" PRIu32 ", lsb: %" PRIu32 "\n", msb, lsb);

  uint32_t *input_ct_msb_buffer = aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * lwe_big_size);
  uint32_t *input_ct_lsb_buffer = aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * lwe_big_size);

  LweCiphertextMutView32 *input_ct_msb_as_mut_view = NULL;
  int input_ct_msb_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_u32(
      default_engine, input_ct_msb_buffer, lwe_big_size, &input_ct_msb_as_mut_view);
  assert(input_ct_msb_mut_view_ok == 0);

  LweCiphertextMutView32 *input_ct_lsb_as_mut_view = NULL;
  int input_ct_lsb_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_u32(
      default_engine, input_ct_lsb_buffer, lwe_big_size, &input_ct_lsb_as_mut_view);
  assert(input_ct_lsb_mut_view_ok == 0);

  int encrypt_msb_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_view_buffers(
      default_engine, lwe_big_sk, input_ct_msb_as_mut_view, msb_encoded, var_big);
  assert(encrypt_msb_ok == 0);

  int encrypt_lsb_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_view_buffers(
      default_engine, lwe_big_sk, input_ct_lsb_as_mut_view, lsb_encoded, var_big);
  assert(encrypt_lsb_ok == 0);

  uint32_t *extract_bits_output_buffer =
      aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * 2 * number_of_bits_per_ct * lwe_small_size);

  LweCiphertextVectorMutView32 *extract_bits_msb_output_as_mut_view = NULL;
  int msb_bits_as_mut_view_ok = default_engine_create_lwe_ciphertext_vector_mut_view_from_u32(
  ...
  LweCiphertextView32 *input_ct_msb_as_view = NULL;
  int input_ct_msb_view_ok = default_engine_create_lwe_ciphertext_view_from_u32(
      default_engine, input_ct_msb_buffer, lwe_big_size, &input_ct_msb_as_view);
  assert(input_ct_msb_view_ok == 0);

  LweCiphertextVectorMutView32 *extract_bits_lsb_output_as_mut_view = NULL;
  int lsb_bits_as_mut_view_ok = default_engine_create_lwe_ciphertext_vector_mut_view_from_u32(
  ...
  LweCiphertextView32 *input_ct_lsb_as_view = NULL;
  int input_ct_lsb_view_ok = default_engine_create_lwe_ciphertext_view_from_u32(
      default_engine, input_ct_lsb_buffer, lwe_big_size, &input_ct_lsb_as_view);
  assert(input_ct_lsb_view_ok == 0);

  int extract_msb_ok = fft_engine_lwe_ciphertext_discarding_bit_extraction_u32_view_buffers(
  ...
  int extract_lsb_ok = fft_engine_lwe_ciphertext_discarding_bit_extraction_u32_view_buffers(
  ...
  uint32_t *output_plaintext_buffer =
      aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * 2 * number_of_bits_per_ct);

  LweCiphertextVectorView32 *extract_bits_output_as_view = NULL;
  int extract_bits_output_view_ok = default_engine_create_lwe_ciphertext_vector_view_from_u32(
  ...
  uint32_t *cbs_vp_output_buffer =
      aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * lwe_big_size * number_of_luts_and_output_cts);

  LweCiphertextVectorMutView32 *cbs_vp_output_as_mut_view = NULL;
  int cbs_vp_output_mut_view_ok = default_engine_create_lwe_ciphertext_vector_mut_view_from_u32(
  ...
  uint32_t *luts = aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * luts_length);

  size_t delta_log_lut = 32 - 2 * number_of_bits_per_ct;

  for (size_t idx = 0; idx < luts_length; ++idx) {
    luts[idx] = ((idx + 1) % ((uint32_t)1 << (2 * number_of_bits_per_ct))) << delta_log_lut;
  }
  ...
  LweCiphertextVectorView32 *cbs_vp_output_as_view = NULL;
  int cbs_vp_output_view_ok = default_engine_create_lwe_ciphertext_vector_view_from_u32(
      default_engine, cbs_vp_output_buffer, lwe_big_size, number_of_luts_and_output_cts,
      &cbs_vp_output_as_view);
  assert(cbs_vp_output_view_ok == 0);

  uint32_t *cbs_vp_decryption_buffer =
      aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * number_of_luts_and_output_cts);

  int result_decrypt_ok = default_engine_decrypt_lwe_ciphertext_vector_u32_view_buffers(
      default_engine, lwe_big_sk, cbs_vp_output_as_view, cbs_vp_decryption_buffer);
  assert(result_decrypt_ok == 0);

  uint32_t expected = val + 1;
  uint32_t rounded =
      closest_representable(cbs_vp_decryption_buffer[0], 1, 2 * number_of_bits_per_ct);
  uint32_t decrypted = rounded >> delta_log_lut;
  printf("decrypted %" PRIu32 ", expected %" PRIu32 "\n", decrypted, expected);
  assert(decrypted == expected);

  destroy_lwe_ciphertext_vector_view_u32(cbs_vp_output_as_view);
  destroy_lwe_ciphertext_vector_mut_view_u32(cbs_vp_output_as_mut_view);
  destroy_lwe_ciphertext_vector_view_u32(extract_bits_output_as_view);
  destroy_lwe_ciphertext_view_u32(input_ct_lsb_as_view);
  destroy_lwe_ciphertext_view_u32(input_ct_msb_as_view);
  destroy_lwe_ciphertext_vector_mut_view_u32(extract_bits_lsb_output_as_mut_view);
  destroy_lwe_ciphertext_vector_mut_view_u32(extract_bits_msb_output_as_mut_view);
  destroy_lwe_ciphertext_mut_view_u32(input_ct_lsb_as_mut_view);
  destroy_lwe_ciphertext_mut_view_u32(input_ct_msb_as_mut_view);
  destroy_lwe_circuit_bootstrap_private_functional_packing_keyswitch_keys_u32(cbs_pfpksk_deser);
  destroy_lwe_circuit_bootstrap_private_functional_packing_keyswitch_keys_u32(cbs_pfpksk);
...
void lwe_cbs_vp_unchecked_view_buffers_test(void) {
  ...
  Buffer cbs_pfpksk_buffer = {.pointer = NULL, .length = 0};
  int cbs_pfpksk_ser_ok =
      default_serialization_engine_serialize_lwe_circuit_bootstrap_private_functional_packing_keyswitch_keys_unchecked_u32(
          default_serialization_engine, cbs_pfpksk, &cbs_pfpksk_buffer);
  assert(cbs_pfpksk_ser_ok == 0);
  ...
  BufferView cbs_pfpksk_buffer_view = {.pointer = cbs_pfpksk_buffer.pointer,
                                       .length = cbs_pfpksk_buffer.length};
  int cbs_pfpksk_deser_ok =
      default_serialization_engine_deserialize_lwe_circuit_bootstrap_private_functional_packing_keyswitch_keys_unchecked_u32(
          default_serialization_engine, cbs_pfpksk_buffer_view, &cbs_pfpksk_deser);
  assert(cbs_pfpksk_deser_ok == 0);
  ...
  LweCiphertextMutView32 *input_ct_msb_as_mut_view = NULL;
  int input_ct_msb_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_unchecked_u32(
      default_engine, input_ct_msb_buffer, lwe_big_size, &input_ct_msb_as_mut_view);
  assert(input_ct_msb_mut_view_ok == 0);

  LweCiphertextMutView32 *input_ct_lsb_as_mut_view = NULL;
  int input_ct_lsb_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_unchecked_u32(
      default_engine, input_ct_lsb_buffer, lwe_big_size, &input_ct_lsb_as_mut_view);
  assert(input_ct_lsb_mut_view_ok == 0);

  int encrypt_msb_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_view_buffers(
      default_engine, lwe_big_sk, input_ct_msb_as_mut_view, msb_encoded, var_big);
  assert(encrypt_msb_ok == 0);

  int encrypt_lsb_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_view_buffers(
      default_engine, lwe_big_sk, input_ct_lsb_as_mut_view, lsb_encoded, var_big);
  assert(encrypt_lsb_ok == 0);
  ...
  int msb_bits_as_mut_view_ok =
      default_engine_create_lwe_ciphertext_vector_mut_view_from_unchecked_u32(
          default_engine, extract_bits_output_buffer, lwe_small_size, number_of_bits_per_ct,
          &extract_bits_msb_output_as_mut_view);
  assert(msb_bits_as_mut_view_ok == 0);

  LweCiphertextView32 *input_ct_msb_as_view = NULL;
  int input_ct_msb_view_ok = default_engine_create_lwe_ciphertext_view_from_unchecked_u32(
      default_engine, input_ct_msb_buffer, lwe_big_size, &input_ct_msb_as_view);
  assert(input_ct_msb_view_ok == 0);
  ...
  LweCiphertextView32 *input_ct_lsb_as_view = NULL;
  int input_ct_lsb_view_ok = default_engine_create_lwe_ciphertext_view_from_unchecked_u32(
      default_engine, input_ct_lsb_buffer, lwe_big_size, &input_ct_lsb_as_view);
  assert(input_ct_lsb_view_ok == 0);
  ...
  int extract_bits_output_view_ok =
      default_engine_create_lwe_ciphertext_vector_view_from_unchecked_u32(
          default_engine, extract_bits_output_buffer, lwe_small_size, 2 * number_of_bits_per_ct,
          &extract_bits_output_as_view);
  assert(extract_bits_output_view_ok == 0);
  ...
  int cbs_vp_ok =
      fft_engine_lwe_ciphertext_vector_discarding_circuit_bootstrap_boolean_vertical_packing_unchecked_u32_view_buffers(
          fft_engine, default_engine, fbsk, cbs_vp_output_as_mut_view, extract_bits_output_as_view,
          luts, luts_length, level_cbs, base_log_cbs, cbs_pfpksk_deser);
  assert(cbs_vp_ok == 0);

  LweCiphertextVectorView32 *cbs_vp_output_as_view = NULL;
  int cbs_vp_output_view_ok = default_engine_create_lwe_ciphertext_vector_view_from_unchecked_u32(
      default_engine, cbs_vp_output_buffer, lwe_big_size, number_of_luts_and_output_cts,
      &cbs_vp_output_as_view);
  assert(cbs_vp_output_view_ok == 0);
  ...
  int result_decrypt_ok = default_engine_decrypt_lwe_ciphertext_vector_unchecked_u32_view_buffers(
      default_engine, lwe_big_sk, cbs_vp_output_as_view, cbs_vp_decryption_buffer);
  assert(result_decrypt_ok == 0);
  ...
  destroy_lwe_ciphertext_vector_view_unchecked_u32(cbs_vp_output_as_view);
  destroy_lwe_ciphertext_vector_mut_view_unchecked_u32(cbs_vp_output_as_mut_view);
  destroy_lwe_ciphertext_vector_view_unchecked_u32(extract_bits_output_as_view);
  destroy_lwe_ciphertext_view_unchecked_u32(input_ct_lsb_as_view);
  destroy_lwe_ciphertext_view_unchecked_u32(input_ct_msb_as_view);
  destroy_lwe_ciphertext_vector_mut_view_unchecked_u32(extract_bits_lsb_output_as_mut_view);
  destroy_lwe_ciphertext_vector_mut_view_unchecked_u32(extract_bits_msb_output_as_mut_view);
  destroy_lwe_ciphertext_mut_view_unchecked_u32(input_ct_lsb_as_mut_view);
  destroy_lwe_ciphertext_mut_view_unchecked_u32(input_ct_msb_as_mut_view);
  destroy_lwe_circuit_bootstrap_private_functional_packing_keyswitch_keys_unchecked_u32(
      cbs_pfpksk_deser);
  destroy_lwe_circuit_bootstrap_private_functional_packing_keyswitch_keys_unchecked_u32(cbs_pfpksk);
...
void lwe_cbs_vp_raw_ptr_buffers_test(void) {
  ...
  int encrypt_msb_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_raw_ptr_buffers(
      default_engine, lwe_big_sk, input_ct_msb_buffer, msb_encoded, var_big);
  assert(encrypt_msb_ok == 0);

  int encrypt_lsb_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_raw_ptr_buffers(
      default_engine, lwe_big_sk, input_ct_lsb_buffer, lsb_encoded, var_big);
  assert(encrypt_lsb_ok == 0);

  uint32_t *extract_bits_output_buffer =
      aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * 2 * number_of_bits_per_ct * lwe_small_size);
  uint32_t *extract_bits_output_buffer_lsb =
      &extract_bits_output_buffer[number_of_bits_per_ct * lwe_small_size];

  int extract_msb_ok = fft_engine_lwe_ciphertext_discarding_bit_extraction_u32_raw_ptr_buffers(
  ...
  int extract_lsb_ok = fft_engine_lwe_ciphertext_discarding_bit_extraction_u32_raw_ptr_buffers(
  ...
  int result_decrypt_ok = default_engine_decrypt_lwe_ciphertext_vector_u32_raw_ptr_buffers(
...
void lwe_cbs_vp_unchecked_raw_ptr_buffers_test(void) {
  ...
  int encrypt_msb_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_raw_ptr_buffers(
      default_engine, lwe_big_sk, input_ct_msb_buffer, msb_encoded, var_big);
  assert(encrypt_msb_ok == 0);

  int encrypt_lsb_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_raw_ptr_buffers(
      default_engine, lwe_big_sk, input_ct_lsb_buffer, lsb_encoded, var_big);
  assert(encrypt_lsb_ok == 0);
  ...
  int cbs_vp_ok =
      fft_engine_lwe_ciphertext_vector_discarding_circuit_bootstrap_boolean_vertical_packing_unchecked_u32_raw_ptr_buffers(
          fft_engine, default_engine, fbsk, cbs_vp_output_buffer, lwe_big_size,
          number_of_luts_and_output_cts, extract_bits_output_buffer, lwe_small_size,
          2 * number_of_bits_per_ct, luts, luts_length, level_cbs, base_log_cbs, cbs_pfpksk_deser);
  assert(cbs_vp_ok == 0);
  ...
  int result_decrypt_ok =
      default_engine_decrypt_lwe_ciphertext_vector_unchecked_u32_raw_ptr_buffers(
          default_engine, lwe_big_sk, cbs_vp_output_buffer, cbs_vp_decryption_buffer,
          number_of_luts_and_output_cts);
  assert(result_decrypt_ok == 0);
