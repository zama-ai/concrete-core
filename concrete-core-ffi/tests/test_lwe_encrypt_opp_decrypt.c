...
void negate_view_buffers_test(void) {
  ...
  int opp_ok = default_engine_discard_opp_lwe_ciphertext_u32_view_buffers(
  ...
  double expected = (pow(2, 32) - 1 - (double)plaintext) / pow(2, SHIFT);
...
void negate_unchecked_view_buffers_test(void) {
  ...
  int opp_ok = default_engine_discard_opp_lwe_ciphertext_unchecked_u32_view_buffers(
      engine, output_ct_as_mut_view, input_ct_as_view);
  assert(opp_ok == 0);
...
void negate_raw_ptr_buffers_test(void) {
  ...
  int opp_ok = default_engine_discard_opp_lwe_ciphertext_u32_raw_ptr_buffers(
...
void negate_unchecked_raw_ptr_buffers_test(void) {
  ...
  int opp_ok = default_engine_discard_opp_lwe_ciphertext_unchecked_u32_raw_ptr_buffers(
      engine, output_ct_buffer, input_ct_buffer, lwe_dimension);
  assert(opp_ok == 0);
