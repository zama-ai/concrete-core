...
void encrypt_add_decrypt_view_buffers_test(void) {
  ...
  uint32_t *input_ct_1_buffer =
      aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * (lwe_dimension + 1));
  uint32_t *input_ct_2_buffer =
      aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * (lwe_dimension + 1));
  uint32_t *output_ct_buffer = aligned_alloc(U32_ALIGNMENT, sizeof(uint32_t) * (lwe_dimension + 1));
  uint32_t plaintext_1 = {((uint32_t)1) << SHIFT};
  uint32_t plaintext_2 = {((uint32_t)2) << SHIFT};

  LweCiphertextView32 *input_ct_1_as_view = NULL;
  int input_ct_1_as_view_ok = default_engine_create_lwe_ciphertext_view_from_u32(
      engine, input_ct_1_buffer, lwe_dimension + 1, &input_ct_1_as_view);
  assert(input_ct_1_as_view_ok == 0);

  LweCiphertextMutView32 *input_ct_1_as_mut_view = NULL;
  int input_ct_1_as_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_u32(
      engine, input_ct_1_buffer, lwe_dimension + 1, &input_ct_1_as_mut_view);
  assert(input_ct_1_as_mut_view_ok == 0);

  LweCiphertextView32 *input_ct_2_as_view = NULL;
  int input_ct_2_as_view_ok = default_engine_create_lwe_ciphertext_view_from_u32(
      engine, input_ct_2_buffer, lwe_dimension + 1, &input_ct_2_as_view);
  assert(input_ct_2_as_view_ok == 0);

  LweCiphertextMutView32 *input_ct_2_as_mut_view = NULL;
  int input_ct_2_as_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_u32(
  ...
  int output_ct_as_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_u32(
      engine, output_ct_buffer, lwe_dimension + 1, &output_ct_as_mut_view);
  assert(output_ct_as_mut_view_ok == 0);

  // We encrypt the plaintext
  int enc_ct_1_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_view_buffers(
      engine, sk, input_ct_1_as_mut_view, plaintext_1, variance);
  assert(enc_ct_1_ok == 0);
  int enc_ct_2_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_view_buffers(
      engine, sk, input_ct_2_as_mut_view, plaintext_2, variance);
  assert(enc_ct_2_ok == 0);

  int add_ok = default_engine_discard_add_lwe_ciphertext_u32_view_buffers(
  ...
  destroy_lwe_ciphertext_view_u32(input_ct_1_as_view);
  destroy_lwe_ciphertext_mut_view_u32(input_ct_1_as_mut_view);
  destroy_lwe_ciphertext_view_u32(input_ct_2_as_view);
  destroy_lwe_ciphertext_mut_view_u32(input_ct_2_as_mut_view);
...
void encrypt_add_decrypt_unchecked_view_buffers_test(void) {
  ...
  LweCiphertextView32 *input_ct_1_as_view = NULL;
  int input_ct_1_as_view_ok = default_engine_create_lwe_ciphertext_view_from_unchecked_u32(
      engine, input_ct_1_buffer, lwe_dimension + 1, &input_ct_1_as_view);
  assert(input_ct_1_as_view_ok == 0);

  LweCiphertextMutView32 *input_ct_1_as_mut_view = NULL;
  int input_ct_1_as_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_unchecked_u32(
      engine, input_ct_1_buffer, lwe_dimension + 1, &input_ct_1_as_mut_view);
  assert(input_ct_1_as_mut_view_ok == 0);

  LweCiphertextView32 *input_ct_2_as_view = NULL;
  int input_ct_2_as_view_ok = default_engine_create_lwe_ciphertext_view_from_unchecked_u32(
      engine, input_ct_2_buffer, lwe_dimension + 1, &input_ct_2_as_view);
  assert(input_ct_2_as_view_ok == 0);

  LweCiphertextMutView32 *input_ct_2_as_mut_view = NULL;
  int input_ct_2_as_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_unchecked_u32(
      engine, input_ct_2_buffer, lwe_dimension + 1, &input_ct_2_as_mut_view);
  assert(input_ct_2_as_mut_view_ok == 0);
  ...
  LweCiphertextMutView32 *output_ct_as_mut_view = NULL;
  int output_ct_as_mut_view_ok = default_engine_create_lwe_ciphertext_mut_view_from_unchecked_u32(
      engine, output_ct_buffer, lwe_dimension + 1, &output_ct_as_mut_view);
  assert(output_ct_as_mut_view_ok == 0);

  // We encrypt the plaintext
  int enc_ct_1_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_view_buffers(
      engine, sk, input_ct_1_as_mut_view, plaintext_1, variance);
  assert(enc_ct_1_ok == 0);
  int enc_ct_2_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_view_buffers(
      engine, sk, input_ct_2_as_mut_view, plaintext_2, variance);
  assert(enc_ct_2_ok == 0);

  int add_ok = default_engine_discard_add_lwe_ciphertext_unchecked_u32_view_buffers(
      engine, output_ct_as_mut_view, input_ct_1_as_view, input_ct_2_as_view);
  assert(add_ok == 0);
  ...
  destroy_lwe_ciphertext_view_unchecked_u32(input_ct_1_as_view);
  destroy_lwe_ciphertext_mut_view_unchecked_u32(input_ct_1_as_mut_view);
  destroy_lwe_ciphertext_view_unchecked_u32(input_ct_2_as_view);
  destroy_lwe_ciphertext_mut_view_unchecked_u32(input_ct_2_as_mut_view);
...
void encrypt_add_decrypt_raw_ptr_buffers_test(void) {
  ...
  // We encrypt the plaintext
  int enc_ct_1_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_raw_ptr_buffers(
      engine, sk, input_ct_1_buffer, plaintext_1, variance);
  assert(enc_ct_1_ok == 0);
  int enc_ct_2_ok = default_engine_discard_encrypt_lwe_ciphertext_u32_raw_ptr_buffers(
      engine, sk, input_ct_2_buffer, plaintext_2, variance);
  assert(enc_ct_2_ok == 0);

  int add_ok = default_engine_discard_add_lwe_ciphertext_u32_raw_ptr_buffers(
...
void encrypt_add_decrypt_unchecked_raw_ptr_buffers_test(void) {
  ...
  // We encrypt the plaintext
  int enc_ct_1_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_raw_ptr_buffers(
      engine, sk, input_ct_1_buffer, plaintext_1, variance);
  assert(enc_ct_1_ok == 0);
  int enc_ct_2_ok = default_engine_discard_encrypt_lwe_ciphertext_unchecked_u32_raw_ptr_buffers(
      engine, sk, input_ct_2_buffer, plaintext_2, variance);
  assert(enc_ct_2_ok == 0);

  int add_ok = default_engine_discard_add_lwe_ciphertext_unchecked_u32_raw_ptr_buffers(
      engine, output_ct_buffer, input_ct_1_buffer, input_ct_2_buffer, lwe_dimension);
  assert(add_ok == 0);
