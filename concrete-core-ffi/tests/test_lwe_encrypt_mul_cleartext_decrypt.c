...
void mul_cleartext_view_buffers_test(void) {
  ...
  uint32_t plaintext = ((uint32_t)2) << SHIFT;

  uint32_t integer_multiplier = 3;
  ...
  int mul_ok = default_engine_discard_mul_lwe_ciphertext_cleartext_u32_view_buffers(
...
void mul_cleartext_unchecked_view_buffers_test(void) {
  ...
  int mul_ok = default_engine_discard_mul_lwe_ciphertext_cleartext_unchecked_u32_view_buffers(
      engine, output_ct_as_mut_view, input_ct_as_view, integer_multiplier);
  assert(mul_ok == 0);
...
void mul_cleartext_raw_ptr_buffers_test(void) {
  ...
  int mul_ok = default_engine_discard_mul_lwe_ciphertext_cleartext_u32_raw_ptr_buffers(
...
void mul_cleartext_unchecked_raw_ptr_buffers_test(void) {
  ...
  int mul_ok = default_engine_discard_mul_lwe_ciphertext_cleartext_unchecked_u32_raw_ptr_buffers(
      engine, output_ct_buffer, input_ct_buffer, lwe_dimension, integer_multiplier);
  assert(mul_ok == 0);
