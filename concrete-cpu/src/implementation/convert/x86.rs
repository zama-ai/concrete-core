...
pub unsafe trait CastInto<T: Copy>: Copy {
    #[inline(always)]
    fn transmute(self) -> T {
        assert_eq!(core::mem::size_of::<T>(), core::mem::size_of::<Self>());
...
fn convert_forward_integer_u64_avx512(
    ...
    assert_eq!(2 * n, out.len());
    assert_eq!(n, in_re.len());
    assert_eq!(n, in_im.len());
    assert_eq!(n, twisties.re.len());
    assert_eq!(n, twisties.im.len());
    ...
    simd.vectorize(
        ...
        || {
            for (out, in_re, in_im, w_re, w_im) in izip!(
                ...
                let out_im = simd._mm512_fmadd_pd(in_re, w_re, simd._mm512_mul_pd(in_im, w_im));
    ...
    simd.vectorize(
        ...
        move || {
            for (out, in_re, in_im, w_re, w_im) in izip!(
                ...
                let out_im = fma._mm256_fmadd_pd(in_re, w_re, avx._mm256_mul_pd(in_im, w_im));
...
fn convert_add_backward_torus_u64_avx512f(
    ...
    let n = out_re.len();
    assert_eq!(n % 8, 0);
    assert_eq!(n, out_re.len());
    assert_eq!(n, out_im.len());
    assert_eq!(2 * n, input.len());
    assert_eq!(n, twisties.re.len());
    assert_eq!(n, twisties.im.len());
