...
__global__ void device_bootstrap_amortized(
  ...
  int8_t *accumulator_mask_decomposed = (int8_t *)selected_memory;
  int8_t *accumulator_body_decomposed =
      (int8_t *)accumulator_mask_decomposed + polynomial_size;
  ...
  for (int iteration = 0; iteration < lwe_dimension; iteration++) {
    ...
    for (int level = 0; level < level_count; level++) {
      ...
      real_to_complex_compressed<int8_t, params>(accumulator_mask_decomposed,
                                                  accumulator_fft);
      ...
      // in the same way
      real_to_complex_compressed<int8_t, params>(accumulator_body_decomposed,
                                                  accumulator_fft);
      synchronize_threads_in_block();
...
__host__ void host_bootstrap_amortized(
  ...
  int SM_FULL = sizeof(Torus) * polynomial_size +   // accumulator mask
                ...
                sizeof(int8_t) * polynomial_size + // accumulator_dec mask
                sizeof(int8_t) * polynomial_size + // accumulator_dec_body
