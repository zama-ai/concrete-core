...
cmux(Torus *glwe_array_out, Torus *glwe_array_in, double2 *ggsw_in,
  ...
  int32_t *glwe_mask_decomposed = (int32_t *)(glwe_sub_body + polynomial_size);
  int32_t *glwe_body_decomposed =
      (int32_t *)glwe_mask_decomposed + (ptrdiff_t)polynomial_size;
  ...
  for (int level = 0; level < level_count; level++) {
    ...
    synchronize_threads_in_block();
    fft<int32_t, params>(glwe_fft, glwe_mask_decomposed);
    ...
    // in the same way
    synchronize_threads_in_block();
    fft<int32_t, params>(glwe_fft, glwe_body_decomposed);
...
void host_cmux_tree(void *v_stream, Torus *glwe_array_out, Torus *ggsw_in,
  ...
  int memory_needed_per_block =
      ...
      sizeof(int32_t) * polynomial_size +     // glwe_mask_decomposed
      sizeof(int32_t) * polynomial_size +     // glwe_body_decomposed
