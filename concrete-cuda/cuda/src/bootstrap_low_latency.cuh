...
__device__ void
mul_ggsw_glwe(Torus *accumulator, double2 *fft, int32_t *glwe_decomposed,
  ...
  real_to_complex_compressed<int32_t, params>(glwe_decomposed, fft);
...
__global__ void device_bootstrap_low_latency(
  ...
  int32_t *accumulator_decomposed = (int32_t *)selected_memory;
...
host_bootstrap_low_latency(void *v_stream, Torus *lwe_array_out,
  ...
  int bytes_needed = sizeof(int32_t) * polynomial_size + // accumulator_decomp
