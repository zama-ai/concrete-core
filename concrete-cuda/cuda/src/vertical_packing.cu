...
/*
 * Performs blind rotation on batch of 32-bit input ggsw ciphertexts.
 * Refer to the equivalent operation on 64 bit inputs for more details.
 */
void cuda_blind_rotate_and_sample_extraction_32(
  ...
  switch (polynomial_size) {
  case 512:
    host_blind_rotate_and_sample_extraction<uint32_t, int32_t, Degree<512>>(
        v_stream, gpu_index, (uint32_t *)lwe_out, (uint32_t *)ggsw_in,
        (uint32_t *)lut_vector, mbr_size, tau, glwe_dimension, polynomial_size,
        base_log, l_gadget, max_shared_memory);
    break;
  case 1024:
    host_blind_rotate_and_sample_extraction<uint32_t, int32_t, Degree<1024>>(
        v_stream, gpu_index, (uint32_t *)lwe_out, (uint32_t *)ggsw_in,
        (uint32_t *)lut_vector, mbr_size, tau, glwe_dimension, polynomial_size,
        base_log, l_gadget, max_shared_memory);
    break;
  case 2048:
    host_blind_rotate_and_sample_extraction<uint32_t, int32_t, Degree<2048>>(
        v_stream, gpu_index, (uint32_t *)lwe_out, (uint32_t *)ggsw_in,
        (uint32_t *)lut_vector, mbr_size, tau, glwe_dimension, polynomial_size,
        base_log, l_gadget, max_shared_memory);
    break;
  case 4096:
    host_blind_rotate_and_sample_extraction<uint32_t, int32_t, Degree<4096>>(
        v_stream, gpu_index, (uint32_t *)lwe_out, (uint32_t *)ggsw_in,
        (uint32_t *)lut_vector, mbr_size, tau, glwe_dimension, polynomial_size,
        base_log, l_gadget, max_shared_memory);
    break;
  case 8192:
    host_blind_rotate_and_sample_extraction<uint32_t, int32_t, Degree<8192>>(
        v_stream, gpu_index, (uint32_t *)lwe_out, (uint32_t *)ggsw_in,
        (uint32_t *)lut_vector, mbr_size, tau, glwe_dimension, polynomial_size,
        base_log, l_gadget, max_shared_memory);
    break;
